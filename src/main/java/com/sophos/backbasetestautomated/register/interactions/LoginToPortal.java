...
public class LoginToPortal implements Interaction {
	...
	public <T extends Actor> void performAs(T actor) {
		...
		if (actualUrl.contains(SECURITY_PROTOCOL)) {
			newUrl = SECURITY_PROTOCOL + actualUrl.split(SECURITY_PROTOCOL)[0] + loginAndUser
					+ actualUrl.split(SECURITY_PROTOCOL)[1];
		} else if (actualUrl.contains(STANDAR_PROTOCOL)) {
			newUrl = STANDAR_PROTOCOL + actualUrl.split(STANDAR_PROTOCOL)[0] + loginAndUser
